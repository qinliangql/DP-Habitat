...
class KinematicHumanoid(MobileManipulator):
    ...
    def __init__(
        ...
        self.height =  1.7 # default height for glb
        # state for ocra
    ...
    def get_state(self,is_full=True):
        self.px , _, self.py = self.base_pos
        rot = mn.Quaternion.from_matrix(self.sim_obj.transformation.rotation())
        angle = rotation2angle(quaternion.quaternion(rot.scalar,*rot.vector))
        angle = (angle+np.pi)%(2*np.pi)
        self.theta = (angle-np.pi)%(2*np.pi)-np.pi
        
        if is_full:
            ocra_state = [self.px,self.py,self.vx,self.vy,self.radius,self.gx,self.gy,self.v_pref,self.theta]
            # ocra_state = [self.px,self.py,self.theta]
        else:
            ocra_state = [self.px,self.py,self.vx,self.vy,self.radius]
    ...
    def set_joint_transform(
        ...
        # print("self.base_pos 1:",self.base_pos)
        self.sim_obj.joint_positions = joint_list
        self.offset_transform = offset_transform
        # print("self.base_pos 2:",self.base_pos)
        add_rot = self.offset_transform_base.inverted()
        final_transform = (base_transform @ add_rot) @ offset_transform
        # print("self.base_pos 3:",self.base_pos)
        self.sim_obj.transformation = final_transform
        # print("self.sim_obj.transformation:",self.sim_obj.transformation)
