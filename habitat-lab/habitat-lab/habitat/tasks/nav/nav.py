...
class Success(Measure):
    ...
    def update_metric(
        ...
        if (
            ...
            and (not self._sim.collision_with_human) 
        ):
            self._metric = 1.0
        else:
            self._metric = 0.0
...
class Collisions(Measure):
    ...
    def reset_metric(self, episode, *args: Any, **kwargs: Any):
        self._metric = {"count": 0, "is_collision": False, "collision_with_human": 0}
    ...
    def update_metric(self, episode, action, *args: Any, **kwargs: Any):
        ...
        if self._sim.collision_with_human:
            self._metric["collision_with_human"] = 1
...
class NavigationTask(EmbodiedTask):
    ...
    def _check_episode_is_active(self, *args: Any, **kwargs: Any) -> bool:
        # 只有在没有被叫停或者没有碰撞的时候才继续
        return (not getattr(self, "is_stop_called", False)) and (not self.sim.collision_with_human)
